...
// Default fallback values if sensors not detected
static const float DEFAULT_PRESSURE_HPA = 1013.25f;
static const float DEFAULT_CO2_PPM = 400.0f;
static const uint16_t DEFAULT_DISTANCE_MM = 0u;
...
void setup() {
  Serial.begin(115200);
  delay(1200);
  Serial.println("=== ESP32 FullV1 + TWAI ===");
...
void loop() {
  ...
  if (millis() - last10 >= 10000) {
...
#endif
#if USE_VL53
      // Always send; use defaults if sensor not detected/valid
      sendVL53Frame();
#endif
    ...
    Serial.print("[FullV1] T1="); Serial.print(last_temp1_c, 2);
...
void sendStatusFrame() {
  ...
  // Compact status: [max1_ok][max2_ok][bme_ok][scd_ok][vl_ok][can_ok][pad][pad]
...
void sendSCD30Frame() {
#if USE_SCD30
  float co2 = DEFAULT_CO2_PPM;
  float t = 0.0f;
  float rh = 0.0f;
  if (scd_ok && scd30.dataAvailable()) {
    co2 = scd30.getCO2();
...
void sendVL53Frame() {
#if USE_VL53
  uint16_t dist = DEFAULT_DISTANCE_MM;
  uint16_t amb = 0;
  uint16_t sig = 0;
  if (vl_ok) {
