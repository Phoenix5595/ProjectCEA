            ...
            if interlock_state == 1:
                ...
                if interlock_load is not None:
                    ...
                    if interlock_load > max_allowed_load:
                        return (
                            False,
                            f"Interlock: {interlock_device} is at {interlock_load:.1f}% (max allowed: {max_allowed_load:.1f}%)",
                        )
                else:
                    ...
                    if then_device == device_name:
                        ...
                        if when_load is not None:
                            if when_load > max_allowed_load:
                                return (
                                    False,
                                    f"Global interlock: {when_device} is at {when_load:.1f}% (max allowed: {max_allowed_load:.1f}%)",
                                )
                        else:
