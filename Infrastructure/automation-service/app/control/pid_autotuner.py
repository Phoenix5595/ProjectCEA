...
class RelayAutoTuner:
    ...
    def update(self, current_value: float, current_time: datetime) -> Tuple[float, Optional[TuningResult]]:
    ...
    def _calculate_tuning(self, method: str = 'some_overshoot') -> Optional[TuningResult]:
        ...
        logger.info(f"Auto-tuning complete: Ku={Ku:.3f}, Tu={Tu:.1f}s, Kp={Kp:.3f}, Ki={Ki:.4f}, Kd={Kd:.3f}")
        
        return TuningResult(
            kp=Kp, ki=Ki, kd=Kd,
            ultimate_gain=Ku, ultimate_period=Tu,
            amplitude=amplitude, tuning_method=method
        )
