...
class EnvironmentState:
    ...
    @property
    def outside_abs_humidity(self) -> Optional[float]:
...
class VPDCascadeController:
    ...
    def __init__(self, vpd_deadband: float = 0.05, kp: float = 20.0, ki: float = 0.5, kd: float = 2.0):
    ...
    def update(self, env: EnvironmentState, target_vpd: float, 
               temp_constraints: TempConstraints, dt: float = 1.0) -> VPDCascadeOutput:
        current_vpd = self.calculate_vpd(env)
        vpd_error = target_vpd - current_vpd
        
        if abs(vpd_error) < self.vpd_deadband:
            return VPDCascadeOutput(current_vpd, target_vpd, vpd_error,
                ActuatorCommand(ActuatorType.NONE, 0, "Within deadband"), [], "VPD OK")
        
        self._integral = max(-self._integral_max, min(self._integral_max, self._integral + vpd_error * dt))
        derivative = (vpd_error - self._last_error) / dt if dt > 0 else 0
        pid_output = min(100.0, abs(self.kp * vpd_error + self.ki * self._integral + self.kd * derivative))
        self._last_error = vpd_error
        
        if vpd_error < 0:
            return self._handle_vpd_too_high(env, temp_constraints, pid_output, current_vpd, target_vpd, vpd_error)
        else:
            return self._handle_vpd_too_low(env, temp_constraints, pid_output, current_vpd, target_vpd, vpd_error)
    ...
    def _handle_vpd_too_low(self, env, constraints, output, current_vpd, target_vpd, vpd_error):
        ...
        if constraints.near_cooling_limit(env.air_temp_c) and outside_drier:
            cmd = ActuatorCommand(ActuatorType.EXHAUST_FAN, output, "VPD low, near cooling, outside drier", 10)
        ...
        # CASE C: Outside drier -> VENTILATE (energy efficient)
            ...
            cmd = ActuatorCommand(ActuatorType.EXHAUST_FAN, output * 0.8, "VPD low, outside drier", 10)
    ...
    def _handle_vpd_too_high(self, env, constraints, output, current_vpd, target_vpd, vpd_error):
        ...
        if constraints.near_cooling_limit(env.air_temp_c):
            secondary.append(ActuatorCommand(ActuatorType.COOLER, output * 0.5, "Supplemental cooling", 5))
