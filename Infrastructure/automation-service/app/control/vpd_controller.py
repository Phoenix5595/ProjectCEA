...
class VPDController:
    ...
    def __init__(
        self,
        leaf_temp_delta: float = DEFAULT_LEAF_DELTA_C,
        kp: float = 5.0,
        ki: float = 0.1,
        kd: float = 0.5,
    ):
    ...
    def calculate_vpd(
        self, air_temp_c: float, humidity_pct: float, leaf_temp_c: float | None = None
    ) -> VPDState:
    ...
    def calculate_target_humidity(
        self, target_vpd: float, air_temp_c: float, leaf_temp_c: float | None = None
    ) -> float:
        if leaf_temp_c is None:
            leaf_temp_c = air_temp_c + self.leaf_temp_delta
        svp_leaf = self.calculate_svp(leaf_temp_c)
    ...
    def update(self, current_vpd: float, target_vpd: float, dt: float = 1.0) -> tuple[float, dict]:
        ...
        self._integral = max(
            -self._integral_max, min(self._integral_max, self._integral + error * dt)
        )
