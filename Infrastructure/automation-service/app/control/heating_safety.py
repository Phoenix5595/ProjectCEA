...
class HeatingFailureSafety:
    ...
    def __init__(self, 
        ...
        self._demand_start_time: Optional[datetime] = None
        self._demand_start_temp: Optional[float] = None
    ...
    def update(self, 
               current_temp: float,
               heater_demand: float,
               current_time: datetime) -> SafetyState:
    ...
    def _set_state(self, state: SafetyState, temp: float, demand: float):
        if state != self._state:
            logger.warning(f"Safety state: {self._state.value} -> {state.value} (temp={temp}°C, demand={demand}%)")
            self._state = state
