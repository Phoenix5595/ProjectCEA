...
async def set_safety_levels(config, dfr0971_manager) -> None:
    ...
    for (board_id, channel), safety_level in safety_levels.items():
        success = dfr0971_manager.set_safety_level(
            board_id, channel, safety_level
        )
        if success:
            logger.info(
                f"Set safety level to {safety_level:.1f}% "
                f"for board {board_id} channel {channel}"
            )
        else:
            logger.warning(
                f"Failed to set safety level "
                f"for board {board_id} channel {channel}"
            )
