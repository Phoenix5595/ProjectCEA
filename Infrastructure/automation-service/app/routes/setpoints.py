...
async def get_all_setpoints(database: DatabaseManager = Depends(get_database)) -> dict[str, Any]:
...
async def get_setpoints(
    ...
    mode: str | None = Query(
        None,
        description="Mode (DAY/NIGHT/TRANSITION). If not specified, returns default/legacy setpoint.",
    ),
...
async def update_setpoints(
    ...
    final_heat = (
        setpoints.heating_setpoint
        if setpoints.heating_setpoint is not None
        else (existing.get("heating_setpoint") if existing else None)
    )
    final_cool = (
        setpoints.cooling_setpoint
        if setpoints.cooling_setpoint is not None
        else (existing.get("cooling_setpoint") if existing else None)
    )
    final_hum = (
        setpoints.humidity
        if setpoints.humidity is not None
        else (existing.get("humidity") if existing else None)
    )
    final_co2 = (
        setpoints.co2 if setpoints.co2 is not None else (existing.get("co2") if existing else None)
    )
    final_vpd = (
        setpoints.vpd if setpoints.vpd is not None else (existing.get("vpd") if existing else None)
    )
    final_ramp_in = (
        setpoints.ramp_in_duration
        if setpoints.ramp_in_duration is not None
        else (existing.get("ramp_in_duration") if existing else None)
    )

    # Validate ramp_in_duration if provided
    if final_ramp_in is not None and (final_ramp_in < 0 or final_ramp_in > 240):
        raise HTTPException(
            status_code=400, detail="ramp_in_duration must be between 0 and 240 minutes"
        )
