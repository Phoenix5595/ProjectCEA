    ...
    async def set_setpoint(
        ...
        try:
            ...
            if humidity is not None:
                await self.set(
                    f"setpoint:{location}:{cluster}:humidity",
                    float(humidity),
                    ttl=ttl,
                )
            if co2 is not None:
            ...
            source_data = {"source": "api", "timestamp": now_ms}
            await self.set(
                f"setpoint:{location}:{cluster}:source",
                json.dumps(source_data),
                ttl=ttl,
            )
        except Exception as e:
            logger.warning(f"StateManager: set_setpoint error for {location}/{cluster}: {e}")
