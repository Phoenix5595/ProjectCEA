        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if row:
                    ...
                    result['day_start_time'] = str(result['day_start_time'])[:5] if result.get('day_start_time') else '06:00'
                    result['night_start_time'] = str(result['night_start_time'])[:5] if result.get('night_start_time') else '18:00'
                    return result
        except Exception as e:
