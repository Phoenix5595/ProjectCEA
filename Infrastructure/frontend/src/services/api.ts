...
class ApiClient implements ApiClientCore {
    ...
    this.automationClient.interceptors.response.use(
      ...
      error => {
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
          ...
          error.message =
            'Network error: Unable to connect to the automation service. Please check if the service is running.'
        }
  ...
  async getMode(
    location: string,
    cluster: string
  ): Promise<{ location: string; cluster: string; mode: string }> {
    const response = await this.automationClient.get(`/api/mode/${location}/${cluster}`)
    return response.data
  }

  async getAllModes(): Promise<
  ...
  async getLightStatus(
    ...
    const response = await this.automationClient.get(
      `/api/lights/${location}/${cluster}/${deviceName}/status`
    )
    return response.data
  }

  async getZoneLightsStatus(
    location: string,
    cluster: string
  ): Promise<{
    lights: Array<{
  ...
  async getLightSchedule(
    ...
    cluster: string,
    deviceName: string
  ): Promise<{ start_time: string; end_time: string; target_intensity: number }> {
    const response = await this.automationClient.get(
      `/api/lights/${location}/${cluster}/${deviceName}/schedule`
    )
    return response.data
  }
  ...
  async updateFlowerCalendarModeTransitions(
    ...
    const response = await this.automationClient.put('/api/calendar/flower-mode-transitions', {
      enabled,
    })
    return response.data
  }
...
// Attach domain methods (devices, sensors, schedules, pid, alarms).
Object.assign(
  ApiClient.prototype,
  deviceMethods,
  sensorMethods,
...
export type { DeviceApi, SensorApi, ScheduleApi, PidApi, AlarmApi, TimelineApi }

export const apiClient = new ApiClient() as ApiClient &
  DeviceApi &
  SensorApi &
  ScheduleApi &
  PidApi &
