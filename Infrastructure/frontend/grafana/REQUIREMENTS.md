...
- **Iskraprojectcea replication alert:** file `Infrastructure/iskra_stack/provisioning/alerting/replication_slot.yaml` emails contact point **Tony** when the primary slot is unhealthy; keep that name in sync with Grafana’s Contact points UI or update the YAML receiver.
