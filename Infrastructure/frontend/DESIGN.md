...
- **Soil extension — `.mon-layout--beds`**: the side column hosts the 4 ft × 4 ft bed schematic cards, so it widens to `minmax(280px, 380px)` while keeping every `mon-side` card rule. Nothing else about the box system changes.
