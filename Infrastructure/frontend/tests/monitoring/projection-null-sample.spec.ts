...
test('renders recorded and projected setpoints with a nullable projected gap', async ({ page }, testInfo) => {
  ...
  await expect(climate.getByRole('button', { name: 'Heating Setpoint (Projected)' })).toBeVisible()
  await expect(climate.getByRole('button', { name: 'Cooling Setpoint (Projected)' })).toBeVisible()
