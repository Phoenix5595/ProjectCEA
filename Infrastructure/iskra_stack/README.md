...
- `docker-entrypoint-replica.sh`: Replica entrypoint; creates minimal `postgresql.conf` / `pg_hba.conf` in PGDATA when missing. Always starts `postgres` with `-c listen_addresses=*` so Grafana and `redis_sync` can connect over the compose network (base backups often inherit `listen_addresses = localhost` from the primary’s data directory stub). For many 1s-refresh panels, it also sets **`max_parallel_workers=0`**, **`max_parallel_workers_per_gather=0`**, and **`jit=off`** so the standby does not peg CPU on parallel workers / JIT while queries pile up (`LWLock: LockManager` in `pg_stat_activity`). Override via env `POSTGRES_MAX_PARALLEL_WORKERS`, `POSTGRES_MAX_PARALLEL_WORKERS_PER_GATHER`, `POSTGRES_JIT` if needed.
