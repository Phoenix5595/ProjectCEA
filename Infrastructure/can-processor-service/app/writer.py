...
class DataWriter:
    ...
    def _start_flush_thread(self):
        ...
        self._flush_thread = threading.Thread(target=self._flush_loop, name="db-batch-flush", daemon=True)
            ...
            try:
                ...
                for item in items:
                    ...
                    if device_name not in self.device_cache:
                        cursor.execute("SELECT device_id FROM device WHERE name = %s", (device_name,))
                        row = cursor.fetchone()
                        if not row:
                            continue
                    ...
                    for sensor_name, value, unit in item.sensors:
                        ...
                        if cache_key not in self.sensor_cache:
                            cursor.execute("SELECT sensor_id FROM sensor WHERE device_id = %s AND name = %s", (device_id, sensor_name))
                            row = cursor.fetchone()
                            if not row:
                                continue
                            self.sensor_cache[cache_key] = row[0]
                        measurements.append((item.timestamp, self.sensor_cache[cache_key], value, None))
                if measurements:
    ...
    def get_stats(self):
        return {"queued": self._queued_count, "flushed": self._flushed_count, 
                "dropped": self._dropped_count, "pending": self._db_queue.qsize()}
    ...
    def close(self):
        ...
        logger.info(f"DB batch stats: queued={stats['queued']}, flushed={stats['flushed']}, dropped={stats['dropped']}")
        if self.db_conn:
            try:
                self.db_conn.close()
            except Exception:
