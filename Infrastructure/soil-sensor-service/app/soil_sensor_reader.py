...
class SoilSensorReader:
    ...
    def read_all_parameters(self) -> dict[str, float] | None:
        """
        ...
        registers = self.modbus.read_holding_registers(
            self.slave_id, self.REGISTERS["temperature"], 4
        )
    ...
    def read_temperature(self) -> float | None:
        ...
        if not self.modbus or not self.modbus.ser or not self.modbus.ser.is_open:
            return None
        registers = self.modbus.read_holding_registers(
            self.slave_id, self.REGISTERS["temperature"], 1
        )
    ...
    def read_humidity(self) -> float | None:
        ...
        if not self.modbus or not self.modbus.ser or not self.modbus.ser.is_open:
            return None
        registers = self.modbus.read_holding_registers(self.slave_id, self.REGISTERS["humidity"], 1)
        return registers[0] * self.SCALING["humidity"] if registers else None

    def read_ec(self) -> float | None:
        """Read only EC"""
        if not self.modbus or not self.modbus.ser or not self.modbus.ser.is_open:
            return None
        registers = self.modbus.read_holding_registers(self.slave_id, self.REGISTERS["ec"], 1)
    ...
    def read_ph(self) -> float | None:
        ...
        if not self.modbus or not self.modbus.ser or not self.modbus.ser.is_open:
            return None
        registers = self.modbus.read_holding_registers(self.slave_id, self.REGISTERS["ph"], 1)
