...
class BackgroundTasks:
    """Manages backend background tasks using worker pattern.

    This class demonstrates the worker pattern for backend services.
    In the future, this can be migrated to use the shared worker framework.
    """
    ...
    def __init__(self):
        ...
        # Sensor location mapping (moved from global scope)
    ...
    async def start(self) -> None:
        ...
        if self._running:
            logger.warning("Backend background tasks already running")
            return

        self._running = True
        self._task = asyncio.create_task(self._sensor_broadcast_loop())
        logger.info("Backend sensor broadcast task started")
    ...
    async def stop(self) -> None:
        ...
        logger.info("Backend sensor broadcast task stopped")

    async def _sensor_broadcast_loop(self) -> None:
        """Sensor broadcast worker - periodically broadcasts sensor data via WebSocket."""
        broadcast_interval = 1.0  # Every second
        consecutive_errors = 0
        max_consecutive_errors = 10
        ...
        while self._running:
            try:
                ...
                # Execute sensor broadcast (worker pattern)
                success = await self._broadcast_sensor_data()
                if success:
                    consecutive_errors = 0
                else:
                    consecutive_errors += 1
    ...
    async def _broadcast_sensor_data(self) -> bool:
        """Broadcast latest sensor data - worker execution method."""
        try:
            # Get all sensor values from Redis
            sensor_values = await get_all_sensor_values()
            if not sensor_values:
            ...
            for sensor_name, value in sensor_values.items():
                try:
                    if sensor_name in self.sensor_location_map:
                        location, cluster = self.sensor_location_map[sensor_name]

                        # Get timestamp for this sensor
                        timestamp = await get_sensor_timestamp(sensor_name)
            ...
            logger.debug(f"Broadcasted {len(sensor_values)} sensor values")
            return True
