...
class SoilSensorScanner:
    ...
    def test_serial_port(self) -> bool:
        ...
        try:
            ...
            print(f"  4. Try different port: /dev/ttyUSB0, /dev/ttyAMA0, /dev/ttyS0")
            return False
    ...
    def scan_bus(self, start_id: int = 1, end_id: int = 254, verbose: bool = False) -> List[int]:
        ...
        try:
            with ModbusRTU(self.port, self.baudrate, timeout=0.5) as modbus:
                for slave_id in range(start_id, end_id + 1):
                    ...
                    if registers is not None:
                        ...
                        print(f"{GREEN}✓ Found sensor at Modbus ID {slave_id} (value: {registers[0]}){NC}")
                    else:
            ...
            print(f"{YELLOW}Make sure the port exists and you have permissions (check dialout group){NC}")
            return []
    ...
    def read_sensor(self, slave_id: int) -> Optional[Dict[str, float]]:
        """
    ...
    def display_reading(self, slave_id: int, readings: Dict[str, float]):
        ...
        print(f"{BLUE}[{timestamp}]{NC} {CYAN}Modbus ID {slave_id}:{NC} "
                ...
                while True:
                    for slave_id in sensor_ids:
                        ...
                        if readings:
                            ...
                            print(f"{RED}[{datetime.now().strftime('%H:%M:%S')}] "
...
Examples:
  ...
  %(prog)s --interval 10
    ...
    parser.add_argument(
        ...
        type=str,
        default='1-254',
        help='Modbus ID range to scan (default: 1-254)'
    )
    ...
    parser.add_argument(
        '--verbose',
        action='store_true',
        help='Verbose output (show detailed scan information)'
    )
    ...
    if args.test:
        ...
        if not found_any:
            ...
            print(f"  1. {YELLOW}Voltage:{NC} Check sensor power supply (should be 5V or 12V depending on sensor)")
            print(f"  2. {YELLOW}Wiring:{NC} Verify A/B wires are connected correctly (not swapped)")
            print(f"  3. {YELLOW}Modbus ID:{NC} Sensor might have different ID (try scanning full range)")
            print(f"  4. {YELLOW}Baudrate:{NC} Sensor might use different baudrate (try 2400, 4800, 9600, 19200)")
            print(f"  5. {YELLOW}Board:{NC} Test with multimeter - check if MAX13487 is getting power")
            ...
            print(f"  - Check sensor documentation for default Modbus ID")
            print(f"  - Verify sensor LED/power indicator is on")
        else:
    ...
    while not found_sensors:
        ...
        if not found_sensors:
            ...
            print(f"  - Wiring connections")
            print(f"  - Sensor power")
