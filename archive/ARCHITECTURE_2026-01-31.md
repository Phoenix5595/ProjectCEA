...
**Last updated (deployed):** 2026-01-31 — ProjectCEA stack (iskra): database replica + Redis + Grafana + redis_sync; Grafana reads Redis for current values, PostgreSQL for time-series.
...
#### ProjectCEA stack (iskra)

- **Purpose**: Database replica + Grafana + Redis on iskra for read-only analytics and live panels without hammering the DB.
- **Docker only on iskra**: One compose stack in `Infrastructure/iskra_stack/`: **projectcea_database** (TimescaleDB streaming standby), **projectcea_redis**, **projectcea_grafana**, **projectcea_redis_sync**. Mothernode runs PostgreSQL, Grafana, Redis natively (systemd); no Docker on mothernode.
- **Replication**: WAL streaming from mothernode (primary) to iskra standby; PGDATA on Storage 1. See `Infrastructure/database-replica/README.md` for mothernode setup.
- **Grafana**: Same dashboards and provisioning as mothernode. Datasources: PostgreSQL → `projectcea_database` (time-series); Redis → `projectcea_redis` (current values). Grafana reads current values from Redis and time-series from PostgreSQL so the DB is not queried every second for “latest”.
- **Redis population**: **projectcea_redis_sync** periodically reads `latest_sensor_values` from the replica DB and writes `sensor:*` / `sensor:*:ts` to Redis. Alternative: configure iskra Redis as replica of mothernode Redis if reachable (e.g. Tailscale).
- **Setup**: See `Infrastructure/iskra_stack/README.md` (Storage 1 path, `.env`, replace standalone database compose if present).
