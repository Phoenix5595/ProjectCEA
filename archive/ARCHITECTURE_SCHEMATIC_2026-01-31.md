...
**Last updated (deployed):** 2026-01-31 — ProjectCEA stack (iskra): database + Redis + Grafana + redis_sync; Grafana reads Redis for current values, PostgreSQL for time-series.
...
flowchart TB
  ...
  subgraph ISKRA["Iskra (Docker only) – ProjectCEA stack"]
    REPLICA["projectcea_database\nstreaming standby\nPGDATA on Storage 1"]
    REDIS_ISKRA["projectcea_redis\ncurrent values"]
    SYNC["projectcea_redis_sync\nDB → Redis"]
    GRAF_ISKRA["Grafana\nPostgreSQL + Redis"]
  end
  ...
  AUTO --> FE
  TSDB --> GRAF
  TSDB -->|"WAL stream"| REPLICA
  REPLICA -->|time-series| GRAF_ISKRA
  REPLICA --> SYNC
  SYNC --> REDIS_ISKRA
  REDIS_ISKRA -->|current values| GRAF_ISKRA
```
...
| Iskra | projectcea_database | Streaming standby (Docker); PGDATA on Storage 1 |
| Iskra | projectcea_redis | Current values for Grafana; populated by redis_sync from replica DB |
| Iskra | projectcea_redis_sync | Periodic latest_sensor_values → Redis (sensor:*, sensor:*:ts) |
| Iskra | projectcea_grafana | Same dashboards as mothernode; PostgreSQL (time-series) + Redis (current) |
...
| UI | Grafana | Dashboards; PostgreSQL (mothernode); iskra stack: PostgreSQL + Redis |
