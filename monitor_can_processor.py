...
DB_CONFIG = "host=localhost dbname=cea_sensors user=cea_user"


def _psql_env() -> dict:
    """Build environment for psql calls without hardcoding credentials."""
    env = os.environ.copy()
    # If POSTGRES_PASSWORD is set, use it via PGPASSWORD (psql will pick it up).
    # Otherwise rely on ~/.pgpass or peer auth.
...
class IncrementalMonitor(BaseMonitor):
    ...
    def update_screen(self):
        ...
        if is_first_update or service_status != self.prev_values.get("service_status"):
            status_text = (
                f"can-processor.service: {GREEN}ACTIVE{NC}"
                if service_status == "active"
                else f"can-processor.service: {RED}INACTIVE{NC}"
            )
        ...
        if is_first_update or messages_key != self.prev_values.get("messages_key"):
            ...
            self.render_two_columns(
                formatted, self.line_positions["messages"], max_message_lines, col_width
            )
