# ESP32 FullV6 - Multi-Node CAN Sensor System with Sensor Reconnection Detection

## Overview
ESP32 FullV6 is based on FullV5 with the addition of **runtime sensor reconnection detection**. The system automatically detects when sensors are reconnected while running and reinitializes them without requiring a board reset. This solves the issue where reconnecting a sensor (like the CO2 sensor) required a manual reset to resume logging.

## New Feature: Sensor Reconnection Detection

### How It Works
- **Periodic checks**: Every 10 seconds, the system checks all sensors that are currently not working
- **Automatic reinitialization**: When a sensor is detected, it's automatically reinitialized
- **Serial notifications**: Reconnection events are logged to the serial monitor
- **No reset required**: Sensors can be reconnected at any time without power cycling the board

### Supported Sensors
All sensors support reconnection detection:
- **MAX31865 (MAX1 & MAX2)**: Checks SPI communication and configuration register
- **BME280**: Checks I2C chip ID and reinitializes at the correct address (0x76 or 0x77)
- **SCD30**: Checks I2C presence and reinitializes with proper settings
- **VL53L0X**: Checks I2C presence and reinitializes

### Example Usage
1. Start the ESP32 with a sensor disconnected (e.g., SCD30)
2. The system will report: `SCD30 not found - will periodically check for reconnection...`
3. Connect the sensor while the board is running
4. Within 10 seconds, you'll see: `[RECONNECT] SCD30 sensor reconnected and reinitialized!`
5. The sensor immediately starts sending data on the CAN bus
...
The default reconnection check interval is 10 seconds. To change it, modify this line:
```cpp
const unsigned long RECONNECT_CHECK_INTERVAL_MS = 10000; // Check every 10 seconds
```
...
The V6 receiver automatically detects missing sensors by:
...
5. **Runtime reconnection**: Automatically detects and reinitializes reconnected sensors
...
- **Wait up to 10 seconds** for automatic reconnection detection if sensor was just connected

### Sensor Not Reconnecting
- Ensure sensor is properly powered and wired
- Check I2C/SPI connections
- Verify sensor addresses match expected values
- Check serial output for reconnection messages
- Increase `RECONNECT_CHECK_INTERVAL_MS` if sensors need more time to stabilize
...
### Normal Operation:
```
=== ESP32 FullV6 Node 1 + TWAI ===
=== With Sensor Reconnection Detection ===
BME280 OK
SCD30 OK  
VL53 OK
TWAI started.
...
### Sensor Reconnection:
```
SCD30 not found - will periodically check for reconnection...
...
[RECONNECT] SCD30 sensor reconnected and reinitialized!
Sensor reconnection check complete - some sensors were reconnected!
```

## Improvements Over V5

- **Runtime sensor reconnection detection**: Sensors can be reconnected without reset
- **Automatic reinitialization**: All sensors are automatically reinitialized when detected
- **Serial notifications**: Clear messages when sensors are reconnected
- **Backward compatible**: Fully compatible with V5 receiver and database schema
- **Low overhead**: Reconnection checks only occur every 10 seconds and only for failed sensors

## Efficiency Features (from V5)
